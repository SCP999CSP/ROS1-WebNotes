export default[ 
    { text: 'RoboParks station',link: 'https://roboparks.com/'},
    { text: 'Home', link: '/' },
    { text: 'Linux', link: '/markdown-examples' },
    { text: 'Python', link: '/markdown-examples' },
    { text: 'OpenCV', link: '/markdown-examples' },
    { text: 'ROS1', link: '/Language/English/1 ROS Overview and Environment Setup' },
    { text: 'Language',
        items:[
            {text: 'le français', link: '/markdown-examples'},
            {text: 'English', link: '/Language/ROS1/English/1 ROS Overview and Environment Setup'},
            {text: '中文', link: '/Language/ROS1/中文/1ROS概述与环境搭建.md'}
        ]
     }
 ];